...
import type { AllErrors, ProductRow, RowErrors } from '../types';

// Thêm import Formik và Form
import { Form, Formik } from 'formik';

// ErrorDisplay (không thay đổi)
...
export default function EditableProductTable({
  ...
  // ... (tất cả các hàm handleInputChange, addRow, deleteRow, v.v. giữ nguyên) ...
  // handleInputChange
  const handleInputChange = (id: string, field: keyof ProductRow, value: string) => {
    ...
    setRows((prev) =>
      prev.map((row) => {
        ...
        if (['regularPrice', 'salePrice', 'discountType'].includes(field as string)) {
  ...
  const deleteRow = (id: string) => {
    setRows((prev) => {
      ...
      if (prev.length === 1 && prev[0].id === id) {
        console.log('>>> [DEBUG][Table] 最後の行が削除されたため、テーブルとjobIdをリセットします');
        setJobId(null);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      try {
        ...
        if (!headers.includes('商品管理番号') || !headers.includes('テンプレート')) {
          toast.error('CSVヘッダーに必要な列（商品管理番号, テンプレート）が含まれていません。');
          return;
        }
        ...
        if (newRows.length > 0) {
          ...
          if (isTableCurrentlyEmpty) {
            ...
            toast.success(`CSVから ${newRows.length} 件の商品を既存のリストに追加しました。`);
          }
        } else {
          toast.warning('CSVファイルの読み込みに失敗したか、有効な商品データが含まれていません。');
        }
      } catch (error) {
  ...
  return (
    // Bọc toàn bộ component bằng Formik
    <Formik
      initialValues={{ rows: rows }} // Cung cấp state `rows` cho Formik
      onSubmit={() => {}} // Không cần submit
      enableReinitialize // Rất quan trọng: Giúp Formik cập nhật khi `rows` thay đổi
    >
      {() => (
        // Thêm tag <Form>
        <Form>
                ...
                <Table.Root className="w-full table-fixed">
                  ...
                  <Table.Body>
                    {rows.map((row, index) => {
                      ...
                      const getErrorMsg = (fieldName: keyof RowErrors): string | undefined =>
                      ...
                      return (
                        <Table.Row key={row.id}>
                          ...
                          {/* Các component Table.InputCell và Table.SelectBox không phải là Formik component, giữ nguyên */}
                          <Table.InputCell
                            id={`productCode-${row.id}`}
                            name={`productCode-${row.id}`} // Giữ nguyên
                            placeholder="番号を入力"
                          ...
                          <Table.Td className="p-0 align-top">
                            <div className={tieredContainerStyle}>
                              <div className="relative border-b border-gray-300">
                                {/* Đây là component TextBox bị lỗi */}
                                <TextBox
                                  type="datetime-local"
                                  label=""
                                  showLabel={false}
                                  id={`startDate-${row.id}`}
                                  // Sửa 'name' để Formik hiểu
                                  name={`rows[${index}].startDate`}
                                  className={cn(
                                    tieredInputStyle,
                                    hasError('startDate') && '!ring-1 !ring-red-500 !ring-inset',
                                  )}
                                  value={row.startDate}
                                  onChange={(e) =>
                              ...
                              <div className="relative">
                                ...
                                <TextBox
                                  ...
                                  className={cn(
                                    tieredInputStyle,
                                    hasError('endDate') && '!ring-1 !ring-red-500 !ring-inset',
                                  )}
                                  value={row.endDate}
                                  onChange={(e) =>
                          ...
                          {/* Giữ nguyên Table.SelectBox */}
                          <Table.SelectBox
                            id={`priceType-${row.id}`}
                            value={row.priceType}
                            onChange={(e) => handleInputChange(row.id, 'priceType', e.target.value)}
                            className={selectCellStyle}
                          >
                            {/* ... options */}
                          ...
                          <Table.InputCell
                            ...
                            className={inputCellStyle}
                            value={row.salePrice}
                            onChange={(e) => handleInputChange(row.id, 'salePrice', e.target.value)}
                          ...
                          <Table.Td className="p-0 align-top">
                            <div className={tieredContainerStyle}>
                              <div className="border-b border-gray-300">
                                {/* Đây là component SelectBox bị lỗi */}
                                <SelectBox
                                  ...
                                  onChange={(e) =>
                                    handleInputChange(row.id, 'discountType', e.target.value)
                                  }
                              ...
                              <TextBox
                                ...
                                className={cn(tieredInputStyle, 'text-center bg-gray-100')}
                                value={row.discount}
                                readOnly
                              />
                            </div>
                          ...
                          <Table.InputCell
                            id={`saleText-${row.id}`}
                            name={`saleText-${row.id}`} // Giữ nguyên
                            placeholder="文言を入力"
                            className={inputCellStyle}
                            value={row.saleText}
                            onChange={(e) => handleInputChange(row.id, 'saleText', e.target.value)}
                            ...
                            <div className={tieredContainerStyle}>
                              <div className="relative border-b border-gray-300">
                                ...
                                <TextBox
                                  ...
                                  // Sửa 'name' để Formik hiểu
                                  name={`rows[${index}].mobileStartDate`}
                                  className={cn(tieredInputStyle)}
                                  value={row.mobileStartDate}
                                  onChange={(e) =>
                                    handleInputChange(row.id, 'mobileStartDate', e.target.value)
                                  }
                                />
                              </div>
                              <div className="relative">
                                ...
                                <TextBox
                                  ...
                                  // Sửa 'name' để Formik hiểu
                                  name={`rows[${index}].mobileEndDate`}
                                  className={cn(tieredInputStyle)}
                                  value={row.mobileEndDate}
                                  onChange={(e) =>
                                    handleInputChange(row.id, 'mobileEndDate', e.target.value)
                                  }
                                />
                              </div>
                            </div>
