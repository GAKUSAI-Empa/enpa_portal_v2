...
import useConsultingRegistrationAPI from './api/useConsultingRegistrationAPI';

const page = () => {
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();
  const { createConsultingRegistration } = useConsultingRegistrationAPI();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setIsLoading(true);
        const resData = await createConsultingRegistration(
          values.companyName,
          values.personName,
          values.email,
            ...
            <div className="flex flex-col justify-center w-full mt-2">
              <div className="flex flex-col items-center justify-center h-full flex-1">
                <div className="w-full xl:max-w-[50%]">
                  <Card>
                    <CardHeader title="[コンサル]新規お申し込みフォーム" />
                    <CardContent>
                      <TextBox
                        id="companyName"
                        name="companyName"
